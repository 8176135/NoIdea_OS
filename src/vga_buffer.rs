...
impl ColorCode {
	const ERROR: Self = Self::new(Color::White, Color::Red);
	...
	fn _foreground(&self) -> Color {
		unsafe { core::mem::transmute(self.0 as u8 & 0b1111) }
	}
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(C)] // Guarantee correct ordering of fields
struct ScreenChar {
	ascii_character: u8,
...
impl ScreenChar {
	const _BLANK: ScreenChar = ScreenChar {
		ascii_character: b' ',
	...
	pub fn write_string(&mut self, s: &str) {
		for byte in s.bytes() {
			match byte {
				...
				_ => self.write_byte(0xfe), // this char: ■
			}
		}
	}
...
use spin::Mutex; // Use spinlock for now until an actual mutex is coded
